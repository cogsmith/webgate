...
App.SNI = function (host, cb) {
	...
	else if (!App.Map[host] && (!host.startsWith('WWW.') || (host.startsWith('WWW.') && !App.Map[host.substr(4)]))) { LOG.WARN(host.substr(4)); console.log(App.Map); console.log(App.Map[host.substr(4)]); console.log(!App.Map[host.substr(4)]); LOG.DEBUG('SNI.Deny: ' + host + ' Not Listed In Routing Map'); cb(null, Error('SNI:NOMAP')); return; }
	// else if ((host.startsWith('WWW.') && !App.Map[host] && !App.Map[host.substr(4)]) || !App.Map[host]) { LOG.WARN(host.substr(4)); console.log(App.Map); console.log(App.Map[host.substr(4)]); console.log(!App.Map[host.substr(4)]); LOG.DEBUG('SNI.Deny: ' + host + ' Not Listed In Routing Map'); cb(null, Error('SNI:NOMAP')); return; }
	else {
